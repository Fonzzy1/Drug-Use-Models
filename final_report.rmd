...
$$ N = S + U + A + R $$
$$ N' = s(S+U+A+R)  -d_SS -d_UU - d_AA - d_RR $$


$$ \frac{d\frac{S}{N}}{dt} = \frac{(-S\pi_{1}N +U\pi_2P  -d_SS + b(S+U+A+R))N-N'S}{N^2} $$ 
$$ \frac{d\frac{U}{N}}{dt} = \frac{(S\pi_1N -U\pi_2P - U\pi_3 - d_UU )N-N'U}{N^2} $$
$$ \frac{d\frac{A}{N}}{dt} = \frac{(U\pi_3 - A\pi_4P + R\pi_5N - d_AA)N-N'A}{N^2}  $$
$$ \frac{d\frac{R}{N}}{dt} = \frac{(A\pi_4P - R\pi_5N - d_RR )N-N'R}{N^2} $$
...
Firstly, one of the main differences between the aforementioned model and ours is the overall use of compartment models/blocks. A similar alteration of the basic SIR model has taken place.In this instance SL(t) represent the population at low risk of being initiated into drug abuse and the class SH(t) represents the population at high risk being initiated into drug abuse. Also Rop(t) stands for rehabilitation as outpatients and Rip(t) stands for the rehabilitation as inpatients.
This form of compartment model may be deemed to be more accurate since in our case we have settled for a simple compartment for the susceptible and recovered population but have made the addicted compartment more intricate. The Hill Function and center manifold theory have been used as well and in our case could have enrichen our own research.
...
## Appendix 3 : Stationary points of Simplified SUAR Model with age

We realize that our 4 equations can take the form 

$$ -S*\pi_1*N+U*\pi_2*P-d_S*S+b*(S+U+A+R)-\frac{N'}{N}*S =0 $$
$$ S*\pi_1*N-U*\pi_2*P-U*\pi_3-d_U*U-\frac{N'}{N}*U=0 $$
$$ U*\pi_3-A*\pi_4*P+R*\pi_5*N-d_A*A-\frac{N'}{N}*A=0 $$
$$ A*\pi_4*P-R\pi_5*N-d_R*R - \frac{N'}{N}*R =0$$

Now adding the first two and the last two we get 

$$ -U*\pi_3-d_S*S-d_U*U+b*(S+U+A+R)-\frac{N'}{N}(*U+S)=0 $$
$$U*\pi_3-d_A*A-d_R*R-\frac{N'}{N}*(A+R)=0 $$

Adding these two again we get

$$ N'-d_S*S-d_U*U-d_A*A-d_R*R+b*(S+U+A+R)=0$$

And simplifying we have

$$ b=s $$

So any point with b=s is a stationary point
